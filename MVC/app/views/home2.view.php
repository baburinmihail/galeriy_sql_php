  ...
  <header class="p-3 bg-dark text-white">
    <div class="container">
      <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
        ...
        <div class="text-end">
          <button type="button" class="btn btn-outline-light me-2" onclick="window.location.href = 'Signin';">Login</button>
          <button type="button" class="btn btn-warning" onclick="window.location.href = 'Registr';">Sign-up</button>
        </div>
      </div>
    </div>
  </header>
    <div class="row align-items-md-stretch">
      <div class="col-md-12">
        <div class="h-100 p-5  rounded-3">
          <form style="display:none" action="Imgcont" method="POST" enctype="multipart/form-data">
            <h2 >Загрузка фото</h2>
      ...
      <div class="col-md-6">
        <div class="h-100 p-5 text-white bg-dark rounded-3">
          ...
          if ($value2['id_img'] == $count){
            ...
            <!--удаление комента-->
          <?php  echo '<a style="display:none" href=http://'.GLOBAL_NAME_SERVER.'/public/DelitComent?id='.$value2['id'].'&comment='.$value2['comment']?>>удалить комент</a>
          <?php
          }else{
            //false
          }  
          ...
          <?php endforeach; ?>
          <!--Форма для комента-->
          <form style="display:none" action="ComentsController" method="POST">
          <input  type="text" class="form-control" name="comment" id="comment" placeholder="text">
          ...
          <a style="display:none" href="<?php echo 'http://'.GLOBAL_NAME_SERVER.'/public/DelitFile?id='.$value['id'] ?>"> удалить картачку</a>
        </div>
      </div>
